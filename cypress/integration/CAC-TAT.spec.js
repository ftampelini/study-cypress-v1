...
import '../support/commands.js'
  ...
  describe('Central de Atendimento ao Cliente TAT', function() {
    ...
    it('CT 02 - Preenche os campos obrigatórios e envia o formulário.', function() {
      ...
      cy.get('#open-text-area').should('be.visible').type('Belo curso, obrigado! Buscando conhecimento.', {delay: 0})
    ...
    it('CT 06 - Preenche e limpa os campos nome, sobrenome, email e telefone.', function() {
      ...
      cy.get('#lastName').type('Caferro Tampelini').should('have.value', 'Caferro Tampelini').clear().should('have.value', '')
      cy.get('#email').type('felipetampelini@gmail.com').should('have.value', 'felipetampelini@gmail.com').clear().should('have.value', '')
      cy.get('#phone').type('44997256970').should('have.value', '44997256970').clear().should('have.value', '')
    })
    ...
    it('CT 09 - Seleciona um produto (YouTube) por seu texto.', function(){
      cy.get('#product').should('be.enabled').select('youtube').should('have.value', "youtube")
    })

    it('CT 10 - Seleciona um produto (Mentoria) por seu texto.', function(){
      cy.get('#product').should('be.enabled').select('mentoria').should('have.value', "mentoria")
    })

    it('CT 11 - Seleciona um produto (Blog) por seu texto.', function(){
      cy.get('#product').should('be.enabled').select('blog').should('have.value', "blog")
    })
    ...
    it('CT 13 - Marca cada tipo de atendimento.', function(){
      cy.get('input[name="atendimento-tat"]').should('have.length', 3).each(function($radio){
        cy.wrap($radio).check()
